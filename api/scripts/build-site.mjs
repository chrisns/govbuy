...
function renderHtml(d) {
  ...
  const noteQs = [
    ...
    { p: "seller", q: "I'm a small business — do frameworks actually favour SMEs, or is it rigged for the big primes?", a: `Mixed, and govbuy is honest about it. PA2023 puts a duty on buyers to consider SME participation, and <b>dynamic markets / DPSs</b> are the lowest-barrier way in (continuous joining, no waiting for a re-tender). A thin track record isn't disqualifying — absence of evidence isn't incapacity. <code>sell</code> ranks the routes you can actually join.` },
    { p: "buyer", q: "We think a contract was awarded unfairly. What are our options?", a: `The ${A(L.pa2023, "PA2023")} <b>8-working-day standstill</b> after the award-decision notice is the window to challenge before signing; beyond it, remedies run through the courts. govbuy can show you the standstill and the award route, but this is procedural information, <b>not legal advice</b> — take proper counsel.` },
    { p: "buyer", q: "We're a fire & rescue service — which frameworks can we use?", a: `Blue-light buyers use <b>BlueLight Commercial</b>, CCS/GCA agreements, and sector consortia — but eligibility is per-framework. Ask <code>buy</code> for your specific need and it returns the routes your organisation type can actually call off, with the official page to confirm eligibility.` },
    { p: "researcher", q: "Which suppliers win the most without competing — by direct award rather than open tender?", a: `<code>supplier</code> reports each firm's <b>competitive-vs-direct mix</b> (pct_direct_award) and customer concentration from real awards — a high direct-award share with one dominant buyer is a concentration signal worth a closer look. All from the 658k-award corpus, CRN-joined.` },
    { p: "seller", q: "A buyer published a pipeline notice for something I sell. How do I get ahead of it?", a: `<code>watch</code> surfaces the forward pipeline with the <b>expected_notice_date</b> — the date the buyer plans to publish the actual tender — so you can engage early (preliminary market engagement) and have your bid ready before it drops. Re-run it to catch new ones.` },
    { p: "buyer", q: "How do I know a framework is still live and not about to expire on me?", a: `<code>framework</code> returns the lifecycle status + the expiry date, and <code>buy</code> flags the act-before window in the route comparison (<i>"expires Oct 2026 — award before then"</i>). Don't sign a call-off that outlives the agreement; govbuy surfaces the runway so you don't get caught.` },
    { p: "researcher", q: "Can I trust these numbers — where does every figure come from?", a: `Yes by design: every £, framework, supplier and URL is <b>verbatim from a source</b> behind a substring gate — govbuy never invents one. Each claim ships its <code>source_url</code> <i>and</i> a Wayback <code>archived_url</code> so the citation survives the page changing. <code>research({status:true})</code> shows the freshness SLA.` },
  ...
  const chat = [];
  for (let i = 0; chat.length < 60 && streams.some((s) => s.length); i++) {
...
// tokens.css only: this site has never had the semantic type layer that
// colors_and_type.css adds, and pulling it in restyles <code>, .label and
// .numeral (156 elements, measured). The Google Fonts <link> in the head below
// is kept because it starts the fetch earlier than an @import nested in inline
// CSS, but its href is derived from tokens.css and the @import stripped, so the
// two can never disagree.
const require_ = createRequire(import.meta.url);
const design = (spec) =>
  readFileSync(require_.resolve(`@chrisns/design/${spec}`), "utf8");
const TOKENS = design("tokens.css");
const FONT_URL = TOKENS.match(
  /@import url\("(https:\/\/fonts\.googleapis\.com[^"]+)"\)/,
)[1];
const CSS = [TOKENS, design("govbuy")]
  .join("\n")
