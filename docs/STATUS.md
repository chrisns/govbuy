...
### Public website + explorable dashboards (DELIVERED)
`https://govbuy.run.cns.me/` now serves a marketing + **explorable-dashboard** site in the cns.me editorial
design system (Fraunces / Hanken Grotesk / JetBrains Mono; the MCP stays at `/mcp`). ACs in
[site-acceptance.md](site-acceptance.md). Self-contained HTML generated by `api/scripts/build-site.mjs`,
which queries `govbuy_public` LIVE (re-runnable, token-free) and bakes the results in: hero + one-command
install, route×reality×statute, the 17 tools by persona, 10 real example questions, and six dashboards —
an **interactive framework-vs-open price explorer** (pick a sector → the median premium updates; IT is the
one sector where call-offs cost *more*), spend-by-channel, the ~£4bn reseller layer, a sortable
top-frameworks table, the 18-month expiry wave, and catalogue composition — plus a plain-English methodology
note and an honest "what it can't do" section. Served from the same Cloud Run service (read boundary
unchanged; `public/` baked into the image). **An adversarial reviewer (sceptical procurement SME + design
critic) verified every headline number against BigQuery to the digit, found no fabrication, was WOWED, and
said it would reach out** — the definition of done.
...
3. `govbuy-ingest refresh` runs on a GitHub Actions cron
   ([ADR-0006](adr/0006-scheduled-github-actions-refresh.md)): deterministic GCA+G-Cloud spine
   nightly (Mon-Sat, no LLM spend), full run including the LLM agentic sources
   (bramble_hub/bloom/ypo/nhs_sbs) weekly on Sunday. Cost-metered, with a liveness check as the
   job's last step.
